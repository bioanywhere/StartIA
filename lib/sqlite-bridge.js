...
// Update columns on every contact row matching a profile URL (enrichment /
// outreach). `fields` is a flat { column: value } map (arrays pre-stringified).
export function mirrorUpdateByUrl(url, fields) {
  return sqliteOp("updateByUrl", { url, fields });
}
