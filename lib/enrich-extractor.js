...
export async function extractProfileFull() {
  ...
  // LinkedIn lazy-renders, and a background/inactive tab renders even later.
  // Wait (up to ~10s) for the name to appear, nudging lazy content by scrolling.
  const nameReady = () => {
    if (document.querySelector("main h1, h1.text-heading-xlarge, h1")) return true;
    for (const b of document.querySelectorAll('script[type="application/ld+json"]')) {
      if (/"@type"\s*:\s*"Person"|"givenName"|"familyName"/.test(b.textContent || "")) return true;
    }
    return false;
  };
  for (let i = 0; i < 20 && !nameReady(); i++) {
    if (i === 3) {
      try {
  ...
  if (
    ...
    /^\s*(sign up|log in|join linkedin|linkedin)\s*\|?\s*(linkedin)?\s*$/i.test(title)
  ) {
