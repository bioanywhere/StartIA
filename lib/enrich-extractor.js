...
export function extractProfileFull() {
  ...
  // ---- 3) DOM sections: locate by anchor id OR localized header text -------
  // LinkedIn's current profile has no id="experience" anchors, and profiles are
  // often non-English, so match section headers by multilingual patterns too.
  const SECTION_PATTERNS = {
    experience: /experien/i, // Experience / Experiencia / Expérience
    education: /educ|formaci[oó]n/i, // Education / Educación / Formación
    skills: /skill|aptitud|habilidad|competenc|conocimiento/i,
    about: /about|acerca|informaci[oó]n|extracto|resumen|présentation/i,
  };
  const headerText = (sec) => {
    const h =
      sec.querySelector(".pvs-header__title span[aria-hidden='true']") ||
      sec.querySelector("h2 span[aria-hidden='true']") ||
      sec.querySelector("h2, [role='heading']");
    return textOf(h);
  };
  const sectionFor = (kind) => {
    const anchor = document.getElementById(kind);
    if (anchor) return anchor.closest("section") || anchor.parentElement;
    const pat = SECTION_PATTERNS[kind];
    for (const sec of document.querySelectorAll("section")) {
      if (pat.test(headerText(sec))) return sec;
    }
    return null;
  };
    ...
    section
      ? [
          ...section.querySelectorAll(
            "li.artdeco-list__item, li.pvs-list__paged-list-item, .pvs-entity, .pvs-list > li, ul > li"
          ),
        ]
      : [];
  ...
  result._debug = {
    ...
    jsonLdPerson,
    // Is this tab logged in? (global nav avatar / "Me" menu vs a Sign-in CTA.)
    loggedIn: !!document.querySelector(
      ".global-nav__me, .global-nav__me-photo, img.global-nav__me-photo, [data-control-name='nav.settings_myself']"
    ),
    signInVisible: /\bsign in\b|\bjoin now\b|iniciar sesi[oó]n|reg[ií]strate/i.test(bodyTxt),
    gatedNote: /sign in to see|inicia sesi[oó]n para ver|see .* full experience|más información/i.test(bodyTxt),
    scrollHeight: document.body.scrollHeight,
    expSection: !!sectionFor("experience"),
    expItems: itemsIn(sectionFor("experience")).length,
    sectionHeaders: [...document.querySelectorAll("section")]
      .map((s) => headerText(s))
      .filter(Boolean)
      .slice(0, 20),
    liCount: document.querySelectorAll("main li, section li").length,
    // A sample of what actually rendered — tells us if Experience text is even
    // present or if it's a gated/sign-in view.
    mainText: ((document.querySelector("main") || document.body).innerText || "").replace(/\s+/g, " ").slice(0, 600),
  };
  return result;
}
